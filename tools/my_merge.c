...
static double geo_pixel_scale = 0;
...
void combine_path(const char *folder, const char *string, char *path);
int copy_tags(TIFF* in, TIFF* out);
...
int mergeTiff(const char* merge_folder, TIFF* output_tif) {
    ...
    if (input_tif == NULL) {
        TIFFWarning("", "Unable to open file: %s", input_file_path);
        exit(-6);
    }

    copy_tags(input_tif, output_tif);
    output_tif->tif_dir.td_imagelength = round(180.0 / geo_pixel_scale);
    output_tif->tif_dir.td_imagewidth = 2 * output_tif->tif_dir.td_imagelength;
    output_tif->tif_dir.td_tilewidth = input_tif->tif_dir.td_imagewidth;
    output_tif->tif_dir.td_tilelength = input_tif->tif_dir.td_imagelength;


    char* file_prefix = _TIFFmalloc(1024);

    tmsize_t input_buffer_size = input_tif->tif_dir.td_imagelength
            * input_tif->tif_dir.td_imagewidth
            * input_tif->tif_dir.td_samplesperpixel
            * (input_tif->tif_dir.td_bitspersample / 8);
    TIFFWarning("", "input file buf size: %lu\n", input_buffer_size);
    void* input_buffer = _TIFFmalloc(input_buffer_size);

    TIFFClose(input_tif);

    int process_tile_count = 0;
    int total_tiles = (output_tif->tif_dir.td_imagelength / output_tif->tif_dir.td_tilelength)
            * (output_tif->tif_dir.td_imagewidth / output_tif->tif_dir.td_tilewidth);
    int progress = 0;
    ...
    for (uint32_t row = 0; row < output_tif->tif_dir.td_imagelength; row += output_tif->tif_dir.td_tilelength) {
        for (uint32_t col = 0; col < output_tif->tif_dir.td_imagewidth; col += output_tif->tif_dir.td_tilewidth) {
            ...
            int ok = readContigTilesIntoBuffer(input_tif,
                                      input_buffer,
                                      input_tif->tif_dir.td_imagelength,
                                      input_tif->tif_dir.td_imagelength,
                                      -1 // not used here.
                                      );

            if (!ok) {
                TIFFWarning("", "failed to read file to buffer: %s", input_file_path);
                exit(-9);
            }

            if (TIFFWriteTile(output_tif, input_buffer, col, row, 0, 0) < 0) {
                TIFFError(TIFFFileName(output_tif),
...
void cp_tags(TIFF *pTiff, TIFF *pTiff1) {

}

void combine_path(const char *folder, const char *file, char *output) {
    const char* sep = "/";
    if (folder[strlen(folder) - 1] == '/') {
        sep="";
    }
    sprintf(output, "%s%s%s", folder, sep, file);
}
...
main(int argc, char* argv[])
{
    ...
    *mp = '\0';
    ...
    if (optind != argc - 2) {
        TIFFWarning("", "should be folder_path out.tif\n");
        exit(-3);
    }

    char* mergeFolder = argv[optind];
    printf("mergeFolder: %s\n", mergeFolder);
    ...
    do {
        ...
        if (!mergeTiff(mergeFolder, out) || !TIFFWriteDirectory(out)) {
            (void) TIFFClose(out);
            return (EXIT_FAILURE);
        }
    } while(0);
    ...
    static const char usage_info[] =
            "Mask tools \n\n"
            "usage: tiffcp [options] tiff_folder_path output.tif\n"
            "where options are:\n"
    ...
    if (out->tif_dir.td_customValueCount == 0)
    {
        ...
        for (int i = 0; i < td_customValueCount; i++) {
            ...
            if (customValue->info->field_tag == 33550) {
                ...
                geo_pixel_scale = ((double *)customValue->value)[0];
            }

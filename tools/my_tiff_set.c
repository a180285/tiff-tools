...
"Examples: \n"
"  -s 33550 8.58306884765625e-05 test.tif ### for pixel scale\n"
"  -s 42113 1048576.0 test.tif ### for no-data\n"
...
{
    const TIFFField *fip;
    int tagIndex = atoi(tagname);

    if (tagIndex == 42113) {
        TIFFField* newFields = _TIFFCreateAnonField(tiff,
                                                    tagIndex,
                                                    TIFF_ASCII);

        int ok = _TIFFMergeFields(tiff, newFields, 1);
        if (!ok) {
            TIFFWarning("TIFF add tag", "Failed to merge tag filed: %d\n", tagIndex);
            exit(-8);
        }
    }

    if( tagIndex > 0 )
        fip = TIFFFieldWithTag(tiff, (ttag_t)tagIndex);
    else
...
main(int argc, char* argv[])
{
    ...
    if (strcmp(argv[arg_index],"-u") == 0 && arg_index < argc-2) {
            ...
            if (TIFFFieldDataType(fip) == TIFF_ASCII) {
                if (TIFFSetField(tiff, TIFFFieldTag(fip), strlen(argv[arg_index]) + 1, argv[arg_index]) != 1)
		       ...
		       || TIFFFieldWriteCount(fip) == TIFF_VARIABLE) {
                ...
                int tagIndex = atoi(tagname);
                if (tagIndex < 0) {
                    printf("Can not convert tagname to int: '%s'\n", tagname);
                    exit(-9);
                }
                TIFFTagValue* customValue = NULL;
                for (int i = 0; i < tiff->tif_dir.td_customValueCount; i++) {
                    customValue = tiff->tif_dir.td_customValues + i;
                    if (customValue->info->field_tag == tagIndex) {
                        break;
                    }
                    customValue = NULL;
                }
                if (customValue == NULL) {
                    printf("Can not find tag in file, tag: '%s'\n", tagname);
                    exit(-9);
                }

                if (tagIndex == 33550) {
                    *(double *)customValue->value = atof(argv[arg_index]);
                    my_fix_tag_33550(customValue->value);
                } else if (tagIndex == 42113) {
                    TIFFField* newFields = _TIFFCreateAnonField(tiff,
                                                                tagIndex,
                                                                TIFF_ASCII);
                    ...
                    char* nodata = argv[arg_index];
                    ok = TIFFSetField(tiff, tagIndex, strlen(nodata) + 1, nodata);

                    TIFFWarning("TIFF copy", "Add tag [%d] to output tiff, status: %d", tagIndex, ok);
                } else {
                    printf("Only support set custom tag 42113 / 33550 now.\n");
                    exit(-9);
                }
                break;
            }
